...
function ubahProduk($id, $post)
{
    ...
    //value 4 = tidak ada data yang diupload.
    $img = $_FILES['gambar'];
    if ($img['error'] === 4) {
        $gambar     = $pd->gambar;
    ...
    $sql = mysqli_query($konek, "UPDATE produk SET nama='$nama', harga='$harga', gambar='$gambar', kategori='$kategori', deskripsi='$deskripsi',createat='$create', updateat='$update' WHERE id_produk = '$id' ");
    if ($sql > 0) {
...
function profil()
{
    ...
    $query = mysqli_query($konek, "SELECT * FROM  users WHERE role='1'");
    $data = [
        'judul' => 'Profil saya',
